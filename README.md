...
- **도시 안 걸음** — 항구에 들어가면 사람이 서고 방향키로 걷는다. 벽에 닿으면 갈 수 있는
  쪽만 살려 미끄러진다. 보는 쪽 넷에 걸음 그림이 둘씩이다.
- **바람·해류** 표를 읽어 상태줄에 낸다.
- 원본 **열여섯 색 팔레트**로 칠한다.

## 자산

`asset/` 에 구워 둔 것이 들어 있어 **게임이 없어도 돈다**(284 KB, 열아홉 개).
...
- **도시 통행 판정** — 밟을 수 있는 칸을 가르는 게임 표를 못 찾았다. 지금은 바닥·길 칩이 쓰는
  색을 모아 **칩 점의 92% 넘게 그 색으로 그린 칩**이면 밟을 수 있다고 본다
  (`PortMap.WalkPurity`). 「넓게 깔린 칩」으로 고르던 것은 길목·모서리가 다 막혀 걷어냈다.
- **건물에 들어가기** — 술집·조선소 따위. 문 칩을 아직 안 갈랐다.
